...
from .serializers import ChanDaoCaseSerializer, ChanDaoProjectSerializer, ChanDaoModularSerializer, \
    ChanDaoCaseStepSerializer
...
class ChanDaoModularViewSet(ModelViewSet):
    ...
    def project_modular(self, request, project_id=None):
        ...
        for modular in modular_all:
            ...
            modular.case_unexecuted_count = ChanDaoCase.objects.filter(modular_id=modular.id,
                ...
                if f:
                    ...
                    for i in range(1, rows):
                        ...
                        case = ChanDaoCase.objects.create(modular_id=modular_id,
                                                          ...
                                                          user_id=user_id,
                                                          result="unexecuted")
                        ChanDaoCaseStep.objects.create(step=row[12], expect=row[13], case_result="unexecuted",
                                                       remarks="", case=case)
        except Exception as e:
            return ApiResponse(status=1, http_status=status.HTTP_500_INTERNAL_SERVER_ERROR, msg="错误",
                               results="导入失败，第{}行数据异常---{}".format(i, e))
